import { Search, Info, Bus, Menu } from 'lucide-react';
...
export function SearchHeader({
  ...
  return (
    <header className="absolute top-4 left-4 right-4 z-20 pointer-events-none flex items-center justify-between gap-3 max-w-4xl mx-auto">
      {/* ── Desktop Top-Left Floating Brand Badge & Drawer Trigger ────────────── */}
      <div className="hidden md:flex pointer-events-auto items-center gap-1.5 shrink-0">
        {onOpenDrawer && (
          <button
            type="button"
            ...
            className="flex items-center justify-center w-12 h-12 rounded-2xl bg-[#182337]/90 border border-white/10 shadow-2xl backdrop-blur-md hover:border-[#F4A100]/40 text-[#FFF8EE]/80 hover:text-[#F4A100] active:scale-[0.98] transition-all"
          >
        ...
        <button
          ...
          className="flex items-center gap-2.5 px-3.5 h-12 rounded-2xl bg-[#182337]/90 border border-white/10 shadow-2xl backdrop-blur-md hover:border-[#F4A100]/40 active:scale-[0.98] transition-all group"
        >
